...
let key = "7c68517e-8447-43e4-8f2a-2a23bb43c7cd";
...
function SortRoutes() {
   let listRoutes = globalListRoutes.map(a =>Object.assign({},
   a));
...
//Сортировка гидов
function SortGuides() {
   let listGuides = globalListGuides.map(a =>Object.assign({},
   a));
...
window.onload = function() {
   ...
   modalSelectTime.addEventListener('change',
   function() {
      costCalculation();
   });
   modalNumberPeople.addEventListener('change',
   function() {
      costCalculation();
   });
   modFiOp.addEventListener('change',
   function() {
      costCalculation();
   });
   modSeOp.addEventListener('change',
   function() {
      costCalculation();
   });
